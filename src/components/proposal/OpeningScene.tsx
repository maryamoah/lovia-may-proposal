...
export function OpeningScene({
  ...
  return (
    ...
    >
      <motion.div
        style={{ y, scale }}
        className="absolute inset-0 bg-[radial-gradient(circle_at_72%_24%,rgba(185,152,91,.22),transparent_30%),radial-gradient(circle_at_20%_75%,rgba(111,36,32,.22),transparent_32%)]"
      />
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="relative mx-auto w-[min(90vw,64rem)] max-w-5xl py-[max(1rem,env(safe-area-inset-top))]"
      >
      ...
      {standalone ? null : (
        <span className="absolute bottom-[max(1.5rem,env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 text-[.62rem] uppercase tracking-[.34em] text-ivory/45">
          Scroll
        </span>
      )}
    </section>
