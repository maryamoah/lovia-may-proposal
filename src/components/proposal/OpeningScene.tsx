...
export function OpeningScene() {
  return (
    <section id="beginning" className="relative grid min-h-dvh place-items-center overflow-hidden bg-espresso px-6 py-24 text-center text-ivory grain">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_28%,rgba(185,152,91,.20),transparent_30%)]" />
      ...
      <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }} className="relative max-w-4xl">
        ...
        <p className="mt-6 text-xs uppercase tracking-[.28em] text-gold/75">{story.opening.date}</p>
      </motion.div>
