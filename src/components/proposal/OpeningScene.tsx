...
export function OpeningScene({ onComplete, standalone = false }: OpeningSceneProps) {
  ...
  return (
    <section id={standalone ? undefined : 'opening'} className="relative grid min-h-dvh place-items-center overflow-hidden bg-espresso px-5 py-20 text-center text-ivory sm:px-6 sm:py-24">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_24%,rgba(185,152,91,.22),transparent_30%),radial-gradient(circle_at_20%_75%,rgba(111,36,32,.22),transparent_32%)]" />
      <div className="absolute inset-0 bg-[linear-gradient(rgba(5,4,3,.18),rgba(27,18,14,.75))]" />
      <div className="particles" aria-hidden="true" />
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={baseTransition} className="relative mx-auto max-w-5xl">
        <p className="eyebrow text-gold">{story.opening.eyebrow}</p>
        <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ ...baseTransition, delay: 0.08 }} className="mx-auto my-7 h-14 w-14 rounded-full border border-gold/35 before:mx-auto before:mt-4 before:block before:h-6 before:w-8 before:rotate-[-20deg] before:rounded-[50%] before:border before:border-gold/60 sm:my-9 sm:h-16 sm:w-16" />
        <div className="space-y-5">
          {openingLines.map((line, index) => (
            <motion.p
              ...
              transition={{ ...baseTransition, delay: 0.16 + index * 0.12 }}
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ ...baseTransition, delay: 0.4 }} className="mt-6 text-xs uppercase tracking-[.24em] text-gold/75">{story.opening.date}</motion.p>
        {standalone ? (
          <motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ ...baseTransition, delay: 0.48 }} onClick={onComplete} className="btn-primary mt-10">
            Begin our story
          </motion.button>
        ) : null}
      </motion.div>
