    ...
    >
      ...
      >
        <div>
          ...
          <motion.h2
            variants={textReveal}
            id="small-things-title"
            className="mt-4 font-serif text-[clamp(2.1rem,5vw,4.1rem)] leading-[1.02] tracking-[-.025em]"
          >
            {story.smallThings.heading}
          </motion.h2>
          <motion.div
            variants={textReveal}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {story.smallThings.lines.map((line) => (
            <motion.p
              key={line}
              variants={textReveal}
              className="story-copy rounded-sm border border-gold/20 bg-cream/55 px-5 py-5 text-base italic leading-7 text-espresso/75 shadow-[0_18px_40px_rgba(27,18,14,.05)] sm:text-lg"
            >
              {line}
            </motion.p>
          ))}
