...
const themeClasses: Record<
  NavigationTheme,
  { container: string; item: string; active: string }
> = {
  light: {
    container:
      "border-espresso/15 bg-ivory/86 shadow-[0_18px_45px_rgba(27,18,14,.12)]",
  ...
  dark: {
    container:
      "border-gold/25 bg-espresso/72 shadow-[0_18px_45px_rgba(0,0,0,.18)]",
...
function getCurrentNavigationState() {
  ...
  const activeTheme = [...story.progressLabels].reverse().find((item) => {
    ...
    return target
      ? target.getBoundingClientRect().top <= viewportMidpoint
      : false;
  })?.theme;
  const theme =
    (themedElement?.dataset.navigationTheme as NavigationTheme | undefined) ??
    activeTheme ??
    "dark";

  const activeLabel = [...story.progressLabels].reverse().find((item) => {
    const target = document.getElementById(item.target);
    return target
      ? target.getBoundingClientRect().top <= viewportMidpoint
      : false;
  ...
  return {
    theme,
    activeTarget: activeLabel?.target ?? story.progressLabels[0]?.target,
  };
}
...
export function StoryProgress({ hidden = false }: StoryProgressProps) {
  ...
  return (
    ...
    >
      <div
        className={`space-y-2 rounded-full border px-3 py-4 backdrop-blur transition-colors duration-300 ${classes.container}`}
      >
